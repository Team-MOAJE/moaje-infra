...
issuer·audience·JWKS·서명 알고리즘·공개 Auth 경로는 회의에서 확정해야 한다. 값이 없으면 검증을 우회하지 않는다. 자세한 입력 안내는 `moaje-infra/docs/learning/infra.md`와 `GatewayJwtConfiguration` 주석에 남긴다. 양쪽 서비스 gRPC mTLS는 유지하며 Auth·Work RPC 규격을 새로 가정하지 않는다.
